...
COPY --chown=algorithm:algorithm model_retrained_15.pth /opt/algorithm/model_retrained_15.pth
...
COPY --chown=algorithm:algorithm method.pdf /opt/algorithm/method.pdf
COPY --chown=algorithm:algorithm metadata.csv /opt/algorithm/metadata.csv

# Install required python packages via pip - please see the requirements.txt and adapt it to your needs
RUN python -m pip install --user -rrequirements.txt -i https://mirrors.aliyun.com/pypi/simple
